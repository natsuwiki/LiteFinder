...
// 版本元数据（简化版，仅包含生物群系相关配置）
const versionMetadata: Record<string, { biomes: { cherry_grove: string; pale_garden_1: string; pale_garden_2: string } }> = {
  "1_19": {
    biomes: {
...
export function getPreset(key: string, mc_version: string) {
  if (key === "minecraft:nether") {
    ...
    const biomes = versionMetadata[mc_version]?.biomes ?? versionMetadata["1_21_7"].biomes;
    const cherry_grove = key === "minecraft:overworld_update_1_20" ? "minecraft:cherry_grove" : biomes.cherry_grove
    const pale_garden = key === "minecraft:overworld_winter_drop" ? "minecraft:pale_garden" : biomes.pale_garden_1
    return OVERWORLD_PRESET(() => cherry_grove, () => pale_garden, () => biomes.pale_garden_2)
  } else {
    return []
  }
}
